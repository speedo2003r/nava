                ...
                <div class="col-12 ">
                    <div class="page-header callout-primary d-flex justify-content-between">
                        ...
                        <button type="button" data-toggle="modal" data-target="#addModel" class="btn btn-primary btn-wide waves-effect waves-light">
                            <i class="fas fa-plus"></i> اضافة حي
                        </button>
                    </div>
                </div>
            </div>
                ...
                <table id="datatable-table" class="table table-striped table-bordered dt-responsive nowrap"  style="width:100%">
                    ...
                    @foreach($states as $ob)
                        <tr>
                            ...
                            <td>{{$ob->title}}</td>
                            <td>{{$ob->city->full_name}}</td>
                            <td>
                                <button class="btn btn-success mx-2"  onclick="edit({{$ob}})" data-toggle="modal" data-target="#editModel"><i class="fas fa-edit"></i></button>
                                <button class="btn btn-danger" onclick="confirmDelete('{{route('admin.states.destroy',$ob->id)}}')" data-toggle="modal" data-target="#delete-model">
                    ...
                    @if(count($states) > 0)
                        <tr>
                            <td colspan="3">
                                <button class="btn btn-danger confirmDel" disabled onclick="deleteAllData('more','{{route('admin.states.destroy',$ob->id)}}')" data-toggle="modal" data-target="#confirm-all-del">
 ...
 <div class="modal fade" id="addModel">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header"><h4 class="modal-title">اضافة حي</h4></div>
...
<div class="modal fade" id="editModel">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            <div class="modal-header"><h4 class="modal-title">تعديل الحي</h4></div>
...
@push('js')
    <script>
        ...
        function edit(ob){
            $('#editForm')      .attr("action","{{route('admin.states.update','obId')}}".replace('obId',ob.id));
            $('#title_ar')    .val(ob.title_ar);
            $('#title_en')     .val(ob.title_en);
            $('#city')     .val(ob.city_id).change;
        }
    </script>
@endpush
